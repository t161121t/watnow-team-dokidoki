...
BEGIN
  ...
  -- features/auth/actions.tsのZodスキーマ(max(50))と同じ上限。このRPCは
  -- authenticatedにEXECUTE権限があり、Server Actionを経由しない直接呼び出しも
  -- 可能なため、アプリ側バリデーションに頼らずDB側でも同じ制約を課す。
  IF char_length(v_nickname) > 50 THEN
    RAISE EXCEPTION 'create_profile: nickname must be 50 characters or fewer';
  END IF;
  ...
  VALUES (auth.uid(), v_nickname, p_avatar_path, now())
