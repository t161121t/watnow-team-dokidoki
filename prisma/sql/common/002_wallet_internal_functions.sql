...
BEGIN
  IF p_amount <= 0 THEN
    RAISE EXCEPTION '_credit_wallet: amount must be positive (got %)', p_amount;
  END IF;
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION '_credit_wallet: wallet not found for group % user %', p_group_id, p_user_id;
  END IF;
...
BEGIN
  IF p_amount <= 0 THEN
    RAISE EXCEPTION '_debit_wallet: amount must be positive (got %)', p_amount;
  END IF;
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION '_debit_wallet: wallet not found for group % user %', p_group_id, p_user_id;
  END IF;
