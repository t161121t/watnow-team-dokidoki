...
| `lib/supabase/middleware.ts` | middleware専用のセッションリフレッシュ（`updateSession`）。`server.ts`は`next/headers`の`cookies()`に依存するためmiddlewareでは使えず、別実装にしている | ルート直下の `middleware.ts` のみ |
